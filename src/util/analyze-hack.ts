...
import { disableLogs } from '/lib/helper.js'
import { AnalyzeHackServer, BurnerServer } from '/types/types.js';
...
export async function main(ns : NS) : Promise<void> {
    ...
    function getRatesAtHackLevel(server: AnalyzeHackServer, player: Player, hackLevel: number) {
    ...
    const unlocked_servers = servers.filter(s => s.requiredHackingSkill <= player.hacking)
        .map(function (server: AnalyzeHackServer) {
