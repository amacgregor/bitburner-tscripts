...
async function solverArrayJumpingGame(ns : NS, arrayData : any) : Promise<Array<string>> {
    await ns.sleep(1000)
...
async function solverGenerateIPs(ns : NS, arrayData : any) : Promise<any> {
    ...
    for (i = 0; i < arrayData.length - 3; i++) {
        for (j = i + 1; j < arrayData.length - 2; j++) {
            for (k = j + 1; k < arrayData.length - 1; k++) {
                const arrayDelimScratch = JSON.parse(JSON.stringify(arrayDelim))
                arrayDelimScratch[i] = "."
                arrayDelimScratch[j] = "."
                arrayDelimScratch[k] = "."
                ...
                let isValid = 1
                for (l = 0; l < 4; l++) {
                    const tempOctet = candidateAddress.split(".")[l]
    ...
    const tempStr = JSON.stringify(validCandidates)
    return tempStr.replace(/\"/g, '')
}
 
async function solverLargestPrime(ns : NS, arrayData : any) : Promise<any> {
    ...
    while (!primeFound) {
        ...
        for (let i = 2; i < Math.sqrt(arrayData); i++) {
            if (!((arrayData / i) - Math.floor(arrayData / i))) {
...
async function solverLargestSubset(ns : NS, arrayData : any) : Promise<any> {
...
async function solverMergeRanges(ns : NS, arrayData : any) : Promise<any> {
...
async function solverSpiralizeMatrix(ns : NS, arrayData : any) : Promise<any> {
    ns.tprint("solverSpiralizeMatrix()")
    await ns.sleep(1000)
    let i, j
 
    const arrayY = arrayData.length
    const arrayX = arrayData[0].length
 
    const loopCount = Math.ceil(arrayX / 2) + 1
    const marginData = [0, 1, 1, 0]
...
async function solverStockTrader(ns : NS, arrayData : any) : Promise<any> {
...
async function solverTrianglePath(ns : NS, arrayData : any) : Promise<any> {
    ...
    const finalRow = arrayData[arrayData.length - 1]
...
async function solverUniquePaths(ns : NS, arrayData : any) : Promise<any> {
    ...
    const k = arrayData[0] - 1 // k
    const ak = arrayData[1] - 1 // n-k
    const n = k + ak // n = k + (n-k);
...
async function solverUniquePathsII(ns : NS, arrayData : any) : Promise<any> {
...
async function solverWaysToExpress(ns : NS, arrayData : any) : Promise<any> {
    ...
    const tempPermutations = Math.pow(4, (arrayData[0].length - 1))
...
export async function main(ns : NS) : Promise<void> {
    ...
    while (listIndex < listServers.length) {
        await ns.sleep(250)
        const listScan = ns.scan(listServers[listIndex])
    ...
    while (true) {
        ...
        const listFiles = ns.ls(listServers[listIndex], ".cct")
 
        for (let z = 0; z < listFiles.length; z++) {
            const inputData = ns.codingcontract.getData(listFiles[z], listServers[listIndex])
            const inputType = ns.codingcontract.getContractType(listFiles[z], listServers[listIndex])
            let outputData
