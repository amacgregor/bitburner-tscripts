...
const getServerByName = (hostname: string) => serverListByFreeRam.find((s) => s.name == hostname)
...
export async function arbitraryExecution(
  ...
  ns.tprint(`Servers by free ram: ${rootedServersByFreeRam.map((svr) => svr.name + " (" + svr.ramAvailable() + ")")}`)
